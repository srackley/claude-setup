...
2. Is there **in-progress work on a branch?** (uncommitted changes, unmerged branch, open PR)
   ...
   - **Answer this from conversation context only.** Do NOT run `git worktree list`, `git status`, or any discovery commands to find out — that adds context bloat before the handoff is written.
...
**Do this before memory updates** — it must be written before anything else can cause compaction.
...
## Step 3 — Output the handoff path
...
- **Running git discovery in Step 1** — use conversation context to determine in-flight work; `git worktree list` and similar add context bloat before the handoff is written
- **Writing memory before the handoff doc** — handoff must be written first; memory updates can cause compaction
